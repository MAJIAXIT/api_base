...
api:
	cd $(HOME_PATH)/api && ENV_FILE=$(HOME_PATH)/$(ENV_FILE) docker compose --env-file=$(HOME_PATH)/$(ENV_FILE) up --build -d

postgres:
	cd $(HOME_PATH)/postgres && ENV_FILE=$(HOME_PATH)/$(ENV_FILE) docker compose --env-file=$(HOME_PATH)/$(ENV_FILE) up --build -d
...
client:
	cd $(HOME_PATH)/client && ENV_FILE=$(HOME_PATH)/$(ENV_FILE) docker compose --env-file=$(HOME_PATH)/$(ENV_FILE) up --build -d
...
ifeq ($(call image_exists,$(API_TEST_IMAGE)),)
	cd $(HOME_PATH)/api && docker build -f Dockerfile.test -t $(API_TEST_IMAGE) .
endif
